/*
 * LogisticRegressionByNonnegativePLBFGSByNonnegativePLBFGS.cpp
 *
 *  Created on: Feb 23, 2014
 *      Author: Mingjie Qian
 */

#include "LogisticRegressionByNonnegativePLBFGS.h"
#include "Matrix.h"
...
LogisticRegressionByNonnegativePLBFGS::LogisticRegressionByNonnegativePLBFGS() {

}

LogisticRegressionByNonnegativePLBFGS::LogisticRegressionByNonnegativePLBFGS(double epsilon) {
	this->epsilon = epsilon;
}

void LogisticRegressionByNonnegativePLBFGS::loadModel(std::string filePath) {

}

void LogisticRegressionByNonnegativePLBFGS::saveModel(std::string filePath) {

}

void LogisticRegressionByNonnegativePLBFGS::train() {
	...
	while (true) {
		NonnegativePLBFGS::run(G, fval, epsilon, W, flags);
		/*disp("W:");
		disp(W);*/
		if (flags[0])
			break;
...
Matrix& LogisticRegressionByNonnegativePLBFGS::predictLabelScoreMatrix(Matrix& Xt) {
...
void LogisticRegressionByNonnegativePLBFGS::computeGradient(Matrix& res, Matrix& A, Matrix& B) {
...
void LogisticRegressionByNonnegativePLBFGS::computeActivation(Matrix& A, Matrix& X, Matrix& W) {
	double** AData = ((DenseMatrix&) A).getData();
	double* ARow = null;
